...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            assert(caller == self.contract_owner.read(),'');

            let inventory_actually = self.inventory.read(product);

            self.inventory.write(product, new_stock + inventory_actually);
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            self.inventory.write(product, inventory_actually - quantity);
        }
        ...
        fn get_stock(self: @ContractState, product: felt252) -> u32 {
            ...
            let inventory = self.inventory.read(product);
            inventory
        }
